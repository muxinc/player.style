import type { SVGProps } from 'react';

/** Brand mark from the Video.js 10 site's `assets/logos/brands/twitch.svg`. */
export default function TwitchLogo(props: SVGProps<SVGSVGElement>) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 268" aria-hidden="true" {...props}>
      <path
        fill="#5a3e85"
        d="M17.458 0L0 46.556v186.201h63.983v34.934h34.931l34.898-34.934h52.36L256 162.954V0zm23.259 23.263H232.73v128.029l-40.739 40.741H128L93.113 226.92v-34.886H40.717zm64.008 116.405H128V69.844h-23.275zm63.997 0h23.27V69.844h-23.27z"
      />
    </svg>
  );
}
