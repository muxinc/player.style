...
/**
 * The Notflix theme around a `Video`, inside a Video.js `VideoPlayer`.
 *
 * @example
 *   ```tsx
 ...
 *
 *   <VideoPlayer poster="poster.jpg" title="Episode 1">
 *     <NotflixSkin>
 *       <Video src="video.mp4" />
 *     </NotflixSkin>
 *   </VideoPlayer>;
 *   ```;
 */
export function NotflixSkin({ children, className, mediaTitle, ...rest }: NotflixSkinProps) {
  return (
    <Container
      className={classNames('media-skin ps-notflix', className)}
      data-theme="notflix"
      data-preset="video"
      {...rest}
    >
      ...
      {/* media-chrome's default gestures and hotkeys, which the original kept. */}
      ...
      {/* media-chrome's default loading spinner, centred and never auto-hidden (`noautohide`). */}
      ...
      <Controls.Root>
        <Controls.Content className="ps-layer">
          {/* The time bar: a full-width scrubber with the remaining time at its end. */}
          <div className="ps-time-bar">
            ...
            <Time.Value className="ps-remaining" type="remaining" toggle />
          </div>
          ...
          <div className="ps-bar">
            ...
            <SeekButton className="ps-button ps-seek-button" seconds={-10}>
            ...
            {/* Volume: a vertical slider rises over the mute button on hover or keyboard focus. */}
            <div className="ps-volume">
              <MuteButton className="ps-button ps-mute-button">
                ...
                <svg className="ps-icon ps-icon-volume-medium" aria-hidden="true" viewBox="0 0 24 24">
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
            ...
            {/* The title: `mediaTitle`, or the player's content title. */}
            <div className="ps-title">{mediaTitle ?? <Title className="ps-title-text" />}</div>

            {/* Subtitles menu. The captions group hides the trigger when the media has no text tracks. */}
            <Menu.Root side="top" align="end">
              <CaptionsRadioGroup.Root>
                <Menu.Trigger className="ps-button ps-captions-button" aria-label="Subtitles">
                  <svg className="ps-icon" aria-hidden="true" viewBox="0 0 24 24">
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                ...
                <Menu.Popup className="ps-menu">
                  <Menu.Content className="ps-menu-content">
                    <div className="ps-menu-header">Subtitles</div>
