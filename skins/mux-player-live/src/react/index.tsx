...
/*
 * Mux Player for live video on Video.js 10, React component: Mux Player's live layout, with the Live indicator leading
 * the title bar and no seek bar, seek buttons, time or playback rate.
 *
 ...
 * `@player.style/mux-player-live/skin.css`.
 */
import {
  AirPlayButton,
  BufferingIndicator,
...
export type MuxPlayerLiveSkinProps = ContainerProps;
...
/**
 * The Mux Player theme for live video, around a `Video`, inside a Video.js `LiveVideoPlayer`.
 *
 * @example
 *   ```tsx
 ...
 *   ```;
 */
export function MuxPlayerLiveSkin({ children, className, ...rest }: MuxPlayerLiveSkinProps) {
  return (
    <Container
    ...
    >
      ...
      <Controls.Root>
        <Controls.Content className="ps-layer">
          {/* The title bar: the Live indicator, then the player's `title` over its own gradient. */}
              ...
              <Menu.Root side="top" align="start">
                <CaptionsRadioGroup.Root>
                  <WithTooltip label="Captions">
                    {/* A plain trigger, as in the HTML element; the glyph follows the group's `data-active`. */}
