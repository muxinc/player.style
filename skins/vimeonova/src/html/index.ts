...
const TAG_NAME = 'vimeonova-skin';

/* The host is a plain box; everything visual lives on `.ps-vimeonova` inside, so this never needs editing per skin. */
...
/**
 * `<vimeonova-skin>`: the Vimeonova theme around a `<video>`, inside a Video.js `<video-player>`.
 *
 * @example
 *   ```html
 ...
 *       <span slot="byline">Byline</span>
 *     </vimeonova-skin>
 *   </video-player>
 *   ```;
 */
export class VimeonovaSkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, VimeonovaSkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: VimeonovaSkinElement;
  }
}
