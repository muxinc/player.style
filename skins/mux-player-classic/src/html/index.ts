/*
 * Mux Player Classic for Video.js 10, HTML element: registers `<mux-player-classic-skin>`.
 *
...
const TAG_NAME = 'mux-player-classic-skin';
...
/**
 * `<mux-player-classic-skin>`: Mux Player's classic theme around a `<video>`, inside a Video.js `<video-player>`.
 *
 * @example
 *   ```html
 *   <video-player>
 ...
 *   ```;
 */
export class MuxPlayerClassicSkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, MuxPlayerClassicSkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: MuxPlayerClassicSkinElement;
  }
}
