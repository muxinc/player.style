 ...
 * through their `@videojs/html/ui/*` entries.
 */
...
const TAG_NAME = 'tailwind-audio-skin';

/* The host is a plain box; everything visual lives on `.ps-tailwind-audio` inside, so this never needs editing per skin. */
...
/**
 * `<tailwind-audio-skin>`: the Tailwind Audio theme around an `<audio>`, inside a Video.js `<audio-player>`.
 *
 * @example
 *   ```html
 *   <audio-player>
 ...
 *   ```;
 */
export class TailwindAudioSkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, TailwindAudioSkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: TailwindAudioSkinElement;
  }
}
