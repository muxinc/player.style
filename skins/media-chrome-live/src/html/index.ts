/*
 * Media Chrome for live video on Video.js 10, HTML element: registers `<media-chrome-live-skin>` for the live-video
 * preset.
 *
 ...
 * The stylesheet is the on-demand skin's (`skins/media-chrome/src/skin.css`), shared by both packages.
 */
...
import styles from '../../../media-chrome/src/skin.css?inline';

const TAG_NAME = 'media-chrome-live-skin';
...
/**
 * `<media-chrome-live-skin>`: Media Chrome's live control bar around a `<video>`, inside a Video.js
 * `<live-video-player>`: play, the LIVE button, mute, volume, captions, picture-in-picture and fullscreen.
 *
 * @example
 *   ```html
 *   <live-video-player>
 ...
 *   ```;
 */
export class MediaChromeLiveSkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, MediaChromeLiveSkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: MediaChromeLiveSkinElement;
  }
}
