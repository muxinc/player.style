/*
 * Sutro for Video.js 10, HTML edition: registers `<sutro-skin>`.
 *
...
const TAG_NAME = 'sutro-skin';

/* The host is a plain box; everything visual lives on `.ps-sutro` inside, so this never needs editing per skin. */
...
/**
 * `<sutro-skin>`: the Sutro theme around a `<video>`, inside a Video.js `<video-player>`.
 *
 * @example
 *   ```html
 *   <video-player>
 ...
 *   ```;
 */
export class SutroSkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, SutroSkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: SutroSkinElement;
  }
}
