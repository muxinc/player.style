/*
 * Video.js 1 for Video.js 10, HTML element: registers `<videojs-1-skin>`.
 *
...
const TAG_NAME = 'videojs-1-skin';

/* The host is a plain box; everything visual lives on `.ps-videojs-1` inside, so this never needs editing per skin. */
...
/**
 * `<videojs-1-skin>`: the original 2010 Video.js skin around a `<video>`, inside a Video.js `<video-player>`.
 *
 * @example
 *   ```html
 *   <video-player>
 ...
 *   ```;
 */
export class Videojs1SkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, Videojs1SkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: Videojs1SkinElement;
  }
}
