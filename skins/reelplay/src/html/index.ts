/*
 * Reelplay for Video.js 10, HTML element: registers `<reelplay-skin>`.
 *
...
const TAG_NAME = 'reelplay-skin';

/* The host is a plain box; everything visual lives on `.ps-reelplay` inside, so this never needs editing per skin. */
...
/**
 * `<reelplay-skin>`: the Reelplay theme around a `<video>`, inside a Video.js `<video-player>`.
 *
 * @example
 *   ```html
 *   <video-player>
 ...
 *   ```;
 */
export class ReelplaySkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, ReelplaySkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: ReelplaySkinElement;
  }
}
