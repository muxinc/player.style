/*
 * Halloween for Video.js 10, HTML element: registers `<halloween-skin>`.
 *
...
const TAG_NAME = 'halloween-skin';

/* The host is a plain box; everything visual lives on `.ps-halloween` inside, so this never needs editing per skin. */
...
/**
 * `<halloween-skin>`: the Halloween theme around a `<video>`, inside a Video.js `<video-player>`.
 *
 * @example
 *   ```html
 *   <video-player>
 ...
 *   ```;
 */
export class HalloweenSkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, HalloweenSkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: HalloweenSkinElement;
  }
}
