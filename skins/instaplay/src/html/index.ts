/*
 * Instaplay for Video.js 10, HTML edition: registers `<instaplay-skin>`.
 *
...
const TAG_NAME = 'instaplay-skin';

/* The host is a plain box; everything visual lives on `.ps-instaplay` inside, so this never needs editing per skin. */
...
/**
 * `<instaplay-skin>`: the Instaplay theme around a `<video>`, inside a Video.js `<video-player>`.
 *
 * @example
 *   ```html
 *   <video-player>
 ...
 *   ```;
 */
export class InstaplaySkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, InstaplaySkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: InstaplaySkinElement;
  }
}
