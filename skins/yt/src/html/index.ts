...
const TAG_NAME = 'yt-skin';

/* The host is a plain box; everything visual lives on `.ps-yt` inside, so this never needs editing per skin. */
...
/**
 * `<yt-skin>`: the YT theme around a `<video>`, inside a Video.js `<video-player>`.
 *
 * @example
 *   ```html
 *   <video-player>
 ...
 *   ```;
 */
export class YtSkinElement extends BaseElement {
...
if (isBrowser && !customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, YtSkinElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [TAG_NAME]: YtSkinElement;
  }
}
