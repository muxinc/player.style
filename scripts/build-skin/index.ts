...
export function createOpenEdition({
  ...
  const preset = /data-preset="audio"/.test(template) ? 'audio' : 'video';
  const player = `${preset}-player`;
  const playerImport = `@videojs/html/${preset}/player`;
  const reactPlayer = preset === 'audio' ? 'AudioPlayer' : 'VideoPlayer';
  const reactMedia = preset === 'audio' ? 'Audio' : 'Video';
  const reactImport = `@videojs/react/${preset}`;
 ...
 *
 * <${player}>
 *   ...skin.html, with your <${preset}> in place of the media placeholder...
 * </${player}>
 */
...
`;
  ...
  if (directive !== "'use client';") throw new Error(`${name}: src/react/index.tsx must start with 'use client'.`);
...
\`\`\`html
<${player}>
  <!-- paste skin.html here and put your <${preset}> where the media placeholder is -->
</${player}>
\`\`\`
...
/** Write `dist/open/` from the skin's sources after the library build. */
  ...
  return {
    ...
    async writeBundle(options) {
      ...
      const files = createOpenEdition({
        name,
        template: await read('src/html/template.html'),
        css: await read('src/skin.css'),
        htmlEntry: await read('src/html/index.ts'),
        reactEntry: await read('src/react/index.tsx'),
      });
 ...
 * Declaration files come from `tsc -p tsconfig.build.json` in the skin's `build` script; `dist/open/` holds the open
 * edition.
 */
