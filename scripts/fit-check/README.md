...
browser context, and the two engines side by side; the whole matrix (18 skins × 2 frameworks × 2 widths × 2 engines)
...
| `fit/outside-player` | A control the skin renders as visible leaves the player's box, whether or not the player clips it away. A control pushed wholly past the edge (the end of a bar wider than the phone) paints nothing and cannot be tapped, so the other rules never see it; on a phone it is simply gone. Rendered as visible means: neither it nor an ancestor is `display: none`; it is `visibility: visible`; its opacity, times its ancestors', is above 0; and neither it nor an ancestor inside the skin root is withdrawn the way skins hide controls the media cannot use (`hidden`, `data-hidden`, `data-availability="unavailable"` or `"unsupported"`). Clipping plays no part. | A slider is judged by its track: its root, not its thumb, and when that root leaves the player but paints no box of its own, what it draws besides its thumb (Microvideo's and Instaplay's scrubbers are transparent roots hanging a few pixels past the bottom edge as hit area round a hairline drawn inside). A control that paints no box of its own (no background, no border) is judged by what it draws: its text, its SVG shapes, its images. X-mas's bauble button is a tall transparent box whose drawing starts inside the player. A box under 2px across (a visually hidden input) is too small to see. |
