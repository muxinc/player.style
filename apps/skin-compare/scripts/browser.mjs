...
/**
 * Launch Chromium the way the harness needs it: autoplay allowed, and outbound requests sent through the sandbox's
 * HTTPS proxy when one is configured, so jsDelivr and remote media resolve.
 */
export async function launchBrowser() {
  ...
  if (proxyServer) {
    // Playwright otherwise forces loopback traffic through the proxy too, which the dev server on 127.0.0.1 must not do.
