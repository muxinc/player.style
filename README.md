...
Third-party skins live under `skins/*`. All 14 classic player.style themes (YT, Sutro, Minimal, Notflix, Vimeonova, Instaplay, Microvideo, Reelplay, Demuxed 2022, Halloween, X-mas, Winamp, Sutro Audio, Tailwind Audio) are ported to Video.js 10 as `@player.style/<name>` 1.x packages, each with an HTML custom element, a React component, and one shared stylesheet, and all of them are in the gallery. The packages are at `1.0.0-alpha.0` and are not published yet: publishing is not wired on `main` (see [Publishing](docs/porting/README.md#publishing)).
